...
import { login, UserDecode } from "../../service/auth";
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      return navigate(response.data.role === 'admin' ? '/admin' : '/');
  ...
  useEffect(() => {
    if(token && decodedData && decodedData.success && userInfo) {
      return navigate(userInfo.role === 'admin' ? '/admin' : '/')
    }
        ...
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              {error && (
                <div className="text-red-800 text text-sm">{error}</div>
              )}
